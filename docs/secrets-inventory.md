...
| `PAYSTACK_WEBHOOK_SECRET`     | `apps/api` only       | Staging / Prod       | Paystack webhook HMAC verification secret              | Paystack Test/Live        |
...
| `FLUTTERWAVE_WEBHOOK_SECRET`  | `apps/api` only       | Staging / Prod       | Flutterwave webhook hash verification secret           | Flutterwave Test/Live     |
| `NEXT_PUBLIC_FLUTTERWAVE_KEY` | Web, Mobile           | Staging / Prod       | Public key for Flutterwave inline checkout             | Flutterwave Test/Live     |
| `TAVILY_API_KEY`              | `apps/api` only       | Staging / Prod       | Tavily search tool integration                         | Tavily Free Tier ($0)     |
